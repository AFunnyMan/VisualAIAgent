...
## 2026-09-10 11:45 +08:00 — behavior-r02补训与新视频独立测试

用户新放入4段development视频。Sol子Agent分别完成姿态、饮水独立标注及评估器开发，root复核开发素材/事件证据并顺序执行本机MPS训练。旧3段+新2段开发素材为train，另外2段完整源视频只在权重、阈值和时序冻结后评估；私人素材、图片、模型均在忽略目录。

实际命令与结果完整保存在[报告](../docs/behavior-training-r02-20260910.md)、[训练日志](../训练日志.md)和[脱敏汇总](evaluations/behavior-20260910-r02.json)。两模型各20轮，官方YOLO26n-cls初始化，固定last.pt，成功导出FP32 ONNX；姿态训练294.78秒，饮水304.92秒。

r01开发验证：起身/坐下各1/1、离座0/1、饮水2/4。新增开发视频随后用于r02训练，因此不再独立；最终两段约68.10秒：起身2/3、坐下2/3、离座2/2、饮水1/3，额外事件0。7张实际事件截图root及独立审查均与事件相符；4个漏事件如实保留。姿态单帧240/241、饮水267/273，后者真实正例仅6/11，不能以97.8%整体准确率宣布可用。

失败归因：一次饮水分类错，一次.25秒饮水无法通过2fps/.5秒确认；两次姿态事件被边界unknown重置切断。未以调低门限或修改人工标签掩盖失败；未用最终测试继续挑模型或调参。当前质量验收未通过，正式页面、物品模型、.env和业务数据库未替换，实时行为与Windows均未验证。

514个标注任务样本PT/ORT类别完全一致，maxabs3.6359e-6；每次视频推理也执行一致性断言。短CPU基准见报告；合成fresh=False无事件，不替代实机故障验收。云端模型调用0。

评估器新增显式split、训练实际来源指纹/逐样本核验、val/test隔离、每类置信门控统计、完整解码与事件一对一计分，以及数据/标注/代码SHA。实际离线回归`.venv/bin/pytest -q`为212 passed/3 deselected（11.09秒）；最后缓存/评分口径调整后相关16项复验通过，Ruff/格式和差异检查通过。


## 2026-09-10 12:41 +08:00 — r02改进分析与存量预测诊断

读取本轮失败报告、状态机、预处理和官方分类/Pose/PyTorch类别权重资料。未修改业务代码或模型；对存量预测事后重放“低置信期保留旧姿态上下文”候选，姿态事件6/8→8/8、额外0，饮水仍1/3。该诊断不等于独立验收，未证明真实遮挡或故障安全；原始r02报告不改写。推理调用0、云端调用0，未运行新训练。

[改进方案](../docs/behavior-improvement-r02.md)明确开发顺序、未知和故障边界、5/10fps及短确认的候选性质、饮水ROI与类别均衡，以及旧最终测试已用于分析后必须新增最终验收素材。现有素材足够启动开发，不把补拍作为开始改进的前置条件。


## 2026-09-10T14:03:01+08:00 — behavior-r03时序改进、ROI试训及标签勘误

- 用户要求开始解决，三个Sol分别实现时序/ROI/人体辅助及审查，root整合并实际执行训练/双模型缓存/评分。新脚本与原r02分离；生产页面、物品模型、.env、业务库不变，云端调用0。
- 原测试饮水片首个“正例”经root连续原画及独立盲审确认为拿杯未到嘴，原基线1/3应修订为1/2。旧原始标签/报告保留，新标注只移除错误区间/事件；此前分类漏检及整体动作0.25s的归因撤回。四片已消耗为development，不再独立。
- ROI实际13:33:21—13:38:24，20轮303.10秒；同5源508图只改ROI，last ONNX导出成功。原组合与ROI组合各10fps 1332帧，PT/ORT类别一致，概率最大差6.23e-6/5.75e-6。人体缓存另以现有官方模型同帧1332次，metadata哈希/全帧键/时间戳校验通过。
...
## 2026-09-10T17:04:09+08:00 — 座位关联改进、同帧对照与300秒相机运行

- 用户要求完善程序后测试。Sol实现座位ROI人体关联，root整合完整候选、有限前后静态图片、实时严格基线与离线分支；另一个Sol只读复审无阻断。强框≥.5建立目标，真实匹配弱框≥.25最多延续.4秒，IoU≥.3、中心距离≤.15，间隔>.25s/故障/缺框/歧义均重置。保留旧权重与v2时间，不把人体框当无遮挡证明。
- 原4开发片同帧重放，新旧均17/17、额外0（±1s）。合成测试能补回弱框unknown过渡后的sat_down，实际fresh=False仍不补事件；这是程序逻辑证据，不能覆盖上次实机漏报。最终pytest269 passed/1 skipped/3 deselected（10.71s），Ruff/165文件格式/diff检查通过。
- 16:57:25启动真实摄像头0，1920×1080/AVFOUNDATION，首末新鲜16:57:34.681—17:02:34.634，17:02:35限时退出0，无worker停止错误。2809 fresh、4 stale安全丢弃、80 startup，9.36fps，新鲜推理p50/p95=73.81/76.497ms。
- 页面已打开并请求用户两轮坐站/离座返回，但会话结束未收到动作完成回复；预览抽查为空座，新旧0事件。状态计数empty2794/unknown15，不能当严格误报率、真实动作召回或坐下修复完成。诊断0组/事件图0张，逐帧候选仍在忽略目录data/acceptance/behavior-seat-live-20260910-session1。测试服务已关闭；下一轮实机动作和反例待用户配合。
- [报告](../docs/behavior-seat-improvement-20260910.md)、[脱敏汇总](evaluations/behavior-seat-20260910.json)记录命令、代码/模型SHA及限制。未训练、未改正式业务/模型/.env、云端调用0；遮挡/USB重插/长时/Windows未验证。

## 2026-09-10 17:52 +08:00 — 用户请求开始实测，session2启动

- 检查8765无监听、工作区干净后，按上一轮相同模型/ROI启动独立session2。首次使用900秒被CLI可选时长校验拒绝（未开相机），随后改为支持的300秒，输出data/acceptance/behavior-seat-live-20260910-session2，正常启动。
- 浏览器已打开真实页面，约59秒时555 fresh、1 stale、当前empty、0事件。已请求用户两次坐站再离开5秒返回；这是启动记录，尚未获得动作完成回复或判定动作结果。服务限时300秒，结果待本次动作后读取summary及逐帧证据；无云端调用或模型修改。

## 2026-09-10 17:56 +08:00 — session2动作完成与失败核对

- 用户确认动作完成。实际首末新鲜17:50:43.023—17:53:36.803，1624 fresh、6 stale、73 startup，9.34fps。根Agent同源POST正常停止，17:53:37.497结束、退出0、worker_stop_error为空；未跑满300秒。新鲜推理p50/p95=73.782/88.638ms，不含过期失败样本。
- 根Agent目检7张事件图和漏报附近7张连续诊断图，新旧7事件完全一致；第一轮坐下成功，第二轮漏sat_down（恢复seated），返回坐下成功，三次起身及离座有记录。准备阶段另有一次坐下。不据少量动作计算总体准确率或误报率，质量未通过。
- 漏报前17:52:47.473帧推理过期，前后新鲜gap约.6405s清空上下文；恢复后standing仅.208s未满足.3s确认，再遇1162两个人体匹配弱框及1163unknown。缺失败内部耗时，不能猜测长尾源；不取消故障保护或现场调参。下一步定位过期路径阶段耗时并评估重复候选。
- 原始7 PNG/70诊断JPG、JSONL和SHA保存在忽略目录；[报告](../docs/behavior-seat-live-session2-20260910.md)和[脱敏汇总](evaluations/behavior-seat-live-session2-20260910.json)保留结果。代码/权重未修改，无新训练、云端调用或重复全量回归；饮水、遮挡反例、USB恢复和Windows未测。

## 2026-09-10 18:18 +08:00 — 长尾诊断与有界并行修复，动作复测待确认

- 用户要求修复。Sol实现worker分段耗时及有界执行器，root整合失败attempt序号绑定、两路本地模型限时并行、运行版本SHA、CLI和对照测试。过期/断流保护、模型权重与确认门限均保留，云端调用0。此为模型执行线程，不新增Agent。
- 原7事件图OpenCV线程比较差异小；ORT 4线程不空转两轮中位62.33/61.48ms，原2线程空转72.78/72.91ms。初次逐位候选相等断言失败，改为明确数值容差重跑通过；所有实际数据在忽略目录harness/artifacts/behavior-latency-20260910。
- 旧配置60秒相机580 fresh/0 stale；仅调线程的中间实机1581 fresh/2 stale，正常停止。诊断定位两次过期主要在模型执行内：人体204.6ms或饮水165.4ms，整个detect271.9/342.9ms，故未停留在线程调参。不能确定具体CPU算子/系统调度根因，也不把平均耗时改善当无故障。
- 最终人体/饮水各单线程无队列，同帧独立job共用120ms等待预算；timeout/busy清除相应辅助信息，主姿态仍须250ms最终fresh检查，迟到结果不复用，关闭有界且错误可见。14张同帧serial/bounded输出一致，中位61.75→44.28ms。最终pytest279 passed/1 skipped/3 deselected（11.04s），Ruff及169文件格式通过；新增卡住辅助任务、缺失信息、无积压、停止与过期诊断用例。
- 最终真实会话18:15:55启动，截至18:18:33约148.53秒1405 fresh、0 stale，人体4timeout/4busy均主帧fresh，detect中位43.37ms/p95 48.31ms。已开页面请求用户动作，截至此记录尚无动作完成回复，0事件不能当质量通过。运行上限300秒，服务与最终summary在后续核对；本条是进行中快照。
...
## 2026-09-10 用户要求重新打开修复版测试页

确认8765无监听后重启独立会话`data/acceptance/behavior-bounded-actions-20260910-184555`，开始时间2026-09-10T10:46:36.975181+00:00（UTC），原bounded参数、300秒上限。浏览器核验running/在座，新鲜62次、故障0；仍待用户动作，后续读取本目录，不混入旧会话。未改代码或模型。

## 2026-09-10 18:51 +08:00 — 修复版用户动作复测完成

- 用户确认实际只做“一轮坐5秒→站3秒→坐5秒，再离开5秒返回坐下”。根Agent目检全部5张事件图和早期5组诊断代表图：起身、坐下、起身离开、离座、返回坐下5/5均记录，新旧人体关联输出相同，无额外事件。不把原计划未执行的第二轮计漏报；本次短流程通过不等于总体准确率或行为验收通过。
...
## 2026-09-11 22:25 +08 — 恢复识别优化、钥匙/合盖训练的数据准备

- 用户报告躺靠误判站起，并恢复性能/物品质量及钥匙、合盖训练，要求汇总采集素材。root核查代码、既有训练和路线，Sol只读复核分类与人体关联机制。三分类和人体框连续性无法保证躺靠坐姿正确；没有本次逐帧画面，原因仍为待验证假设，不宣称已复现或修复。
- 新增[补拍清单](../docs/recognition-data-collection-20260911.md)：七组开发素材与另录独立验收、正常姿态及困难负例、钥匙/合盖语义、数据隔离和后续实验顺序。参考Ultralytics官方训练及数据划分资料；具体分钟数为首批预算，不是质量保证。
- 本次只完成调查、范围更新和采集文档；未训练、未修改运行程序/权重/.env、未打开或录制摄像头、未运行API或软件回归。新样本训练与实机验收待素材，性能专项尚未继续执行，不能写为已完成第一二项。


## 2026-09-11 新增视频质量初检

- 用户提交development新素材后，检查本日7段视频，ffprobe总376.069秒、1080p/H.264/标称60fps。OpenCV全段顺序解码、3秒联系表人工目检及3张原尺寸细查，保存SHA和本机诊断图于data/quality-review-20260911。帧数估计与读取合计差10帧，未据此认定损坏；不是逐帧动作标注或模型评分。
- 姿态、坐站、钥匙、开合、饮水反例和杯手机素材可作首批开发，最后116秒含后靠/空座可候选整片留出；未见明确验收分组。已询问84—96秒后靠是否原误判姿态，答复待确认。没有训练或使用留出预测调参。
- [质量报告](../docs/new-video-quality-20260911.md)记录逐片观察、可用性和最小补拍建议。未改程序/权重/配置，未开摄像头，云调用0；本次仅素材初检，不代表识别质量通过。


### 新素材姿态真值确认补记

用户确认23-05-02约84—96秒抱臂后靠为原失败姿态，另要求覆盖不抱臂直接后躺坐姿。该片转为开发素材，撤销候选独立留出安排，避免训练/验收混用。只更新真值与数据用途记录，尚未训练或声称视频误判复现。


### 新增独立验收片

用户新增验收2026-09-11 23-08-00.mkv，约176.7秒1080p/标称60fps，已记录SHA和holdout用途。仅16点抽样检查输入质量，未推理或训练；必须在development批量选材时排除此片及派生帧。细节见新视频质量报告补记，不将登记写成验收通过。


## 2026-09-11 23:19 +08 — 开发训练开始与配套程序验证

- 用户明确开始开发训练。三Sol并行准备姿态r04、单类钥匙、局部合盖数据，MPS训练排队；尚无新训练结果。新8源SHA与development/holdout用途冻结，新增source-policy按内容拒绝验收、重命名验收及未注册源。
- root新增独立LaptopTimeline：启动闭合无动作、稳定状态去重、故障/未知/场景变化清基线、明确partial证据有限桥接；尚未接真实模型或正式规则UI，不宣称合盖已可用。
- 正式BehaviorDetector关闭未使用的诊断帧复制/哈希，实验默认保留，事件截图与时效检查不变。300次1080p复制/hash微基准旧中位2.85ms/p95 5.56ms/max16.04ms，跳过该分支接近计时开销；只说明消除该项开销，不代表摄像头长尾修复完成。脱敏结果在evaluations/behavior-diagnostic-copy-20260911.json。
- 实际定向31passed/6.42秒；随后全套pytest326passed、1skipped、4deselected/25.96秒，含当前已落盘的数据准备测试。训练Agent后续改动仍需再次相应验证。未开摄像头，未调用云API，未替换生产权重。


## 2026-09-11 23:49 +08 — 首批训练与数据审查中间结果

- 姿态r04固定20轮完成，七段新开发735帧与旧496帧合并，共1231帧。关键两段开发324抽样均通过原0.75/0.2拒识门；旧r02后靠24/24判standing，新r04为24/24 seated。此为参与训练的视频回归，不是独立质量结论；正式10fps动作回放与独立验收尚在后续。
- 合盖r01全图二分类CPU开发验证69/79正确、7未知、3open错closed；root对23-02-14单片10fps回放469样本，0事件，未通过开合事件要求。三类r02增加明确partial证据，正在训练。
- 钥匙标框复核发现空桌面错标及框过宽，原CPU探测/中断训练作废；root逐图检查26+4帧后冻结30帧r02（11正例/19无可见）。有效12轮仍未收敛、低置信严重；进一步CPU60轮候选在开发中，不改.35阈值。详见[审查](code_review/recognition-training-20260911.md)，错误产物未启用。
- 新增逐样本独立姿态验收工具，人工真值先冻结SHA，检验视频注册为holdout且不在候选训练/选优来源，保留人体支持和动作证据。没有读取验收预测用于调参。
- 本次全套pytest实际336passed、1skipped、4deselected/14.79秒；之后模型清单重复类别检查定向5passed/1.28秒，ruff全库通过。一次误指不存在test_behavior.py的命令未运行测试，已换真实文件重跑，不能计为通过。后续Agent改动继续验证。云API调用0，未开摄像头，未替换生产模型。


## 2026-09-12 00:14 +08 — 姿态独立验收与工具复核

- r04姿态固定候选在全部开发回归后冻结，root于00:00:38—00:04:25运行`scripts.evaluate_posture_holdout`：1767采样中1699明确稳定真值全部接受且正确，另68过渡/未标注不计分类准确率。伸展20与抱臂后靠30采样均在座；原r02这50样本全误站。独立事实见[汇总](evaluations/posture-r04-independent-20260912.json)与[报告](../docs/behavior-posture-r04-20260911.md)。
- 动作按事前固定窗口、无额外容差计4/5、0FP，漏left_seat。118.8秒unknown且人体支持false清前态，119.4秒已有空座状态但未补离座事件；没有更改门槛/预期或以此片重新选优，保持实验候选，不能关闭动作质量验收。
- Sol只读独立审查通过姿态评估的数据隔离/评分/时间语义；发现通用lid回放缺少强制来源用途，root将该入口限定注册development并增加改名holdout拒绝测试。新独立lid入口单独校验人工真值SHA、模型清单SHA及训练/选优来源隔离；尚未运行独立lid预测。
- 全套pytest338passed、1skipped、4deselected/16.05秒；新增独立工具来源保护后定向8passed/1.12秒，差异检查通过。后续钥匙/合盖最后候选尚在执行，参数和结果继续单独记录。未开摄像头、无云API调用、未改生产默认权重。

## 2026-09-12 00:31 +08 — 本轮训练收尾与用户标注复核

- 最终结果汇总见[训练报告](../docs/recognition-training-results-20260912.md)：姿态独立稳定分类1699/1699，动作4/5、0误事件；钥匙r04开发val在既定0.35阈值0/5召回；合盖r04全部开发3761采样仅1次事件。钥匙与合盖未通过开发门槛，未执行其独立验收预测，所有候选均未替换正式默认模型。
- 最后全套验证341passed、1skipped、4deselected/13.67秒；训练隔离环境的test_train_behavior_model另15passed/2.13秒；ruff与差异检查通过。未调用云API，未执行真实摄像头或Windows验收。
- 用户要求查看人工标注。导出当前三个最终数据集的标注展示图到本机忽略目录data/标注复核-20260912，分新姿态735、旧姿态496、钥匙34、笔记本340；独立验收图片不混入训练复核。图片显示实际标签、来源和时间，钥匙框来自最终dataset-r03清单，笔记本蓝框明确为ROI而非物品标框。用户核对期间不启动后续训练，发现错误后另行版本化修正。

## 2026-09-12 — 根据用户复核取消笔记本半开状态

- 用户明确仅要开着/合上/未知，完全关上才算合上。更新数据准备与模型类别校验，移除状态机partial分支；未知和故障不跨越补事件。
- 新r05标注保留原版本，将58张明确partial并入open：340张中open186、closed154。原unknown/未标注不变；新数据硬链接原始图片并逐张SHA核验。旧三类查看图移到本机data/笔记本旧三类标注-20260912，当前标注复核页面改为二类，私人图不提交。待用户复核后重训，当前未替换权重，不能宣称精度提升已通过。
- 修改后全套测试342passed、1skipped、4deselected/13.84秒，新增短暂合盖噪声不触发及移除partial拒绝测试；ruff发现一处行长已修复，复查通过。此为程序回归，未运行新模型精度验证。

## 2026-09-12 — 用户确认钥匙可见标注并询问漏检原因

- 用户确认当前“钥匙可见”标注没有问题；此确认不扩大为全部负例或独立验收已确认。核对既有证据：r03低分诊断11/11真实目标有IoU≥0.5候选，但10/11低于0.35；r04训练6/10、分组开发val0/5且有4重复框。两轮证据分别陈述，不把r03分数移作r04结果。
- 实际框按1920→640缩放计算：训练10正例框宽20—65、高18—57像素；val5正例均约28.3×47.7且为同一视频相近位置的边缘截断。小尺寸、少量且相关的样本是合理风险，未做消融不能判定唯一根因。PT/ONNX既有一致性不支持导出丢失解释。阈值可在开发数据上校准，不应视0.35为不可改变标准；仍需完整负例/连续视频误报评估及独立验收，不能只看五帧改阈值宣布通过。本次未训练或改正式阈值。

## 2026-09-12 — 用户暂时取消钥匙

- 按最新指令暂停全部钥匙训练、优化、补采、验收与接入，保留本机素材及历史实验，不删除。当前钥匙本就未通过开发验收、未启用为正式功能，因此无需移除正式功能或更改生产权重。后续聚焦姿态与笔记本二态分类/未知保护；钥匙恢复须由用户后续指令决定。

## 2026-09-12 00:44—00:55 +08 — 继续剩余工作

- 按用户最新授权继续，最高Sol并行：笔记本r05训练/开发验证，离座证据修复，笔记本正式可选接入；root进行饮水/物品复核、负载验证工具及整合审查。任务边界见docs/remaining-work-20260912.md。
- root人工顺序0.5秒复核23-03-14饮水开发片，18点旧模型诊断发现2个靠脸误报、2个到嘴未接受；据明确区间提取189帧（33饮水、156非饮水），合并旧508为697帧，隔离MPS20轮训练中。未知边界不标，未读取holdout预测。训练入口/参数写在本机training-manifest，不改正式模型。
- 同片旧场景微调物品候选有杯漏检、重复框及错误phone，未据此切换生产模型。一开始使用生产默认清单加载候选因清单不存在被正确拒绝，随后实验模式显式校验既有候选SHA后运行，不绕过生产加载保护。
- 新增scripts/check_shared_replay.py使用真实共享Runtime和真实模型、仅替换为原速视频传输；验证来源隔离与故障恢复补丁的3项测试通过/1.81秒。修正SharedFrameSource来源标签传递，回放不可标为真实摄像头。

## 2026-09-12 01:10—01:23 +08 — 动作修复、候选与正式接入回归

- 最终收紧退出证据实现复算原失败片：5/5预期动作、0额外。两段开发动作10/10、8/8；五段纯在座2043采样0事件。该旧验收片用于定位修复后属于开发回归，不再称新独立验收。版本与边缘/多候选保护见docs/behavior-exit-evidence-20260912.md。
- 饮水r03训练20轮697帧完成，训练拟合697/697，PT/ONNX一致。10fps正式时序开发回放4TP/0FP/0FN（旧4/2/0）。先冻结真值、权重和训练来源后完成饮水留出检查1/0/0；仅一段一次正动作，不能推断日常零误报，姿态链曾使用此片也不称整产品独立验收。细节与命令入口见docs/drinking-training-20260912.md，结果evaluations/drinking-20260912-r03.json。
- 合盖r05/r06二分类均完成。r06仅把同区间采样4fps加密10fps，850帧拟合与ONNX一致；新二类完整人工动作真值先冻结，开发r05为4/8、r06为6/8，均0额外，仍漏两次合盖。未读笔记本holdout、未默认启用，不继续无依据增加轮数。缺少明确absent/完整遮挡素材，已向用户请求补拍。
- 笔记本可选模块、SQLitev4备份迁移、独立事件/证据、UI/七工具/规则完成。审查修复验收记录未绑定模型SHA、事务失败内存前态和重复/逆序观察断链问题；presence与分类非法分数均拒识。正式启用仍依赖未完成的可见性/转场质量验收，不把接口通过当模型通过。
- root全套pytest中间检查381passed、1skipped、5deselected/13.95秒；之后审查修改另跑定向，最终数另记。饮水冻结/来源与回放来源测试5passed/1.56秒。
- 01:16:47—01:21:48，`.venv/bin/python -m scripts.check_shared_replay`使用23-03-14开发原速循环、r04姿态+r03饮水、真实Runtime+物品模型运行300秒：物品300fresh；行为2996fresh、3stale；单一source实例且open一次；正常stop/close，Agent运行0、规则0。物品推理中位114.00/p95 170.74/max453.31ms；行为回调完成年龄中位55.15/p95 79.00/max312.90ms（含保存回调，不能等同接受推理时年龄）；回调max132.10ms；进程峰值638.94MiB。训练已停止，期间有少量边界测试、页面重启及本机应用活动；不是独占硬件基准、真实USB或长期稳定验收。完整结果留本机data/acceptance/shared-replay-r04-r03-20260912。
- root真实浏览器发现旧8501进程缓存造成新增laptop_panel ImportError，正常停止确认的旧项目进程并重启。刷新后正式页面正常，笔记本显示未知/未验收/在场未确认，未开启摄像头。生产数据迁移沿程序备份契约，未清空历史。
- 真实千问受控笔记本工具测试在网络受限环境返回model_request_failed；提权重试被自动审批拒绝，理由为缺少本次文字外发的明确授权。已请求用户允许仅发送构造的状态文字和随机证据编号；未绕过拒绝，真实API测试暂未通过。
- 本机标注复核新增饮水189并更新笔记本r06全850，共2304条。旧r05 340移入独立历史文件夹保留。所有r06原图SHA与清单一致，holdout来源0，私图未提交。

## 2026-09-12 01:23—01:30 +08 — 最后边界修复

- 笔记本r06两次漏合盖只读定位为24.7/44.3秒各单个unknown，未重新推理或改真值。新增独立可见证据下的单帧桥接：前后各150ms、确认时间仍完整500ms、同一转场最多桥接一次。无可见证据、两次不确定、过期、断流或场景变化仍立即断链。Capability验收绑定timing；Detector/Store/Timeline统一语义。此为程序与受控失败测试，不把全程可见假设当真实模型结果，r06实际分数仍6/8。
- 最终停止来源修正后另跑60秒真实Runtime开发视频：物品60fresh、行为601fresh、0stale，含停止的全部观察来源为replay；单source/open一次、正常close。脱敏两次负载记录见evaluations/shared-runtime-20260912.json。
- 桥接前全套386passed、1skipped、5deselected/13.89秒，训练隔离环境test_train_behavior_model为15passed/1.99秒。桥接及其Store接线后定向42passed/1.96秒，最终全套继续核对后记录。Ruff与diff检查通过。

## 2026-09-12 01:32 +08 — 整合验证结论
...
## 2026-09-16 18:29—18:37 +08 — 修正本机行为测试配置

- 用户要求先修正模型配置测试。通过 `load_behavior_manifest` 校验姿态 r04（b46aeb38f1dc）和饮水 r03（3b3cb250670a）的实际权重；将本机 .env 的 VAA_BEHAVIOR_ENABLED、VAA_BEHAVIOR_POSTURE_MANIFEST、VAA_BEHAVIOR_DRINKING_MANIFEST 显式保存为此实验组合，凭据未修改、不提交 .env。未改变物品 YOLO 或全局候选默认策略。
- 原页面操作后数据库仍显示旧 r02，未据输入框宣称切换成功。停止旧服务，执行 `.venv/bin/streamlit run app.py --server.address 127.0.0.1 --server.port 8501 --server.headless true` 后在页面启动观察。18:36:35 数据库当前行为记录 running/fresh=1，实际版本 `behavior-r03:p=b46aeb38f1dc:d=3b3cb250670a:person=9c60d351bb28`，error=NULL、posture=empty；18:37:20 页面行为统计显示空座，累计有效空座47秒。版本前缀 behavior-r03 是流水线标签，实际姿态/饮水身份以两个权重哈希为准。
- 页面已切到行为与统计，请用户配合正常坐姿→后靠→离开→返回验证。本次只确认加载与实时输出，尚未获得本轮动作确认，不宣称坐姿、饮水或杯子质量通过。旧模型产生的当日历史统计保留，不能作为新模型统计质量证据。
...
## 2026-09-17 21:30—22:03 +08 — 已确认数据的R04/R05训练与验证

- 用户回复“确认没问题，请开始训练”。核对87张人工复核图的标签/尺寸/SHA全部对应冻结train，新51同采集组；合公共96为183train/42val/4历史test，旧178条分割/标签不变。独立Sol只读复核无跨组泄漏，final-test未引用/读取。底边不明银白物沿用用户确认的未标注状态，不虚构其类别。
- 21:31:09—21:43:38，隔离MPS R04完成30轮与ONNX导出，748.24秒，原官方起点、lr0 .0002、选26轮。新51图杯22/27→27/27、手机27/45→40/45，手机FP1→0；这些新图已用于训练，只是拟合/开发回归。旧val18杯9/9手机6/6不变，旧四图手机4/4→0/4；诊断框仍定位正确但置信度降低，不能因新图变好就部署。
- 21:46:37—21:55:05，针对退步完成R03权重起点R05，20轮、lr0 .0001、507.75秒，选15轮。训练结束前另预定评估最终20轮，不扫描中间epoch或加入旧四图训练/选优。两版均新杯26/27手机38/45零额外、旧手机2/4，未优于R04的新图效果，完整保留不掩盖。
- 实际调用`scripts/train_scene_model.py`的--check-only及两次训练、`scripts/evaluate_scene_model.py`三候选各219输入；去重前及部署后PT/ONNX全部通过。真实MemoryStore/检测器对旧3视频逐候选61采样回放，均空场0事件、杯片1杯出现、同框各1出现、停止无错误missing；非独立事件准确率，无真实USB/API/Windows。R05评估部分并行，时间不作独占性能基准。
- R04剩余5手机FN为018/029/030/031/050遮挡/窄条/截断；旧四图置信度下降与R05部分恢复见[训练结果](../docs/scene-r04-r05-results-20260917.md)。本机生成三份逐图对照目录，R04入口`data/物品R04训练对照-20260917/新旧模型对照.html`。三候选、全部分组计数/模型SHA与回放经Sol独立复核通过，脱敏[评估记录](evaluations/scene-r04-r05-20260917.json)。
- 本轮仅训练/产物/文档，无业务源码改动，不将此前400项pytest当新跑结果。数据校验、预检查、真实训练、导出比较和真实模型回放均实跑，diff检查通过。未改.env或正式权重；R04仅为当前新图实验优先，生产批准false。停止无依据加轮，下一步是未参与训练的新连续同场景或真实操作验证，仍不宣称模型质量全部修复。


## 2026-09-20 18:40 +08 — 手机范围调整与新增视频检查

- 用户明确忽略靠桌沿、部分出画、手遮挡及两手机同框漏检，已写入AGENTS/GOALS并更新PLANS；不再将这些手机漏检作为修复、补采或验收阻塞，历史结果保留，未扩展豁免到正常单手机/杯/误报。
- 实际用 `.venv/bin/python` 调用OpenCV顺序解码本日4段视频，1920×1080名义60fps、约163秒，保存本机SHA/实际帧数/逐秒缩略图；人工审查均匀概览及关键尾段。素材覆盖空场、杯、单手机及杯手机同框，但移出后的空窗短，未覆盖足量五秒缺失/返回事件。容器帧数与解码数小幅差异单独记录，未据此断言损坏。
- [检查报告与最小补录](../docs/scene-video-review-20260920.md)：现有素材可开始开发比较；补开发/独立验收两段常规放入移出，每类各10轮、在场/离场各8—10秒。无需重拍既有素材或补手机豁免情况。本轮未训练/运行准确率评估、未动生产配置；私人视频/缩略图不提交。文档diff检查，未为文档变更重跑模型测试。保留此前工作树变更。


## 2026-09-20 19:02 +08 — 补录检查、真实模型开发回放与标签防错

- 用户补录两片并要求可用后继续。实际顺序解码新增杯约102.6秒/6155帧、手机133秒/7980帧，冻结SHA并人工检查；独立Sol不看预测审查稳定真值区间。杯4次完整移出，手机3次物理移出（2适用＋1桌沿豁免），不能包装成各10次或开发/验收两套。
- 实际 `.venv/bin/python data/scene-eval-20260920/run.py`：6片、R03/R04/R05各401个1Hz输入，经正式检测器＋杯复查与真实事件机，固定0.35；未训练/扫阈值/读final-test。杯46稳定在场/36不在采样均正确类别输出，4次移出全记录；手机第二次正常移出三模型均无missing。可视化定位底部银白角phone候选持续干扰，已向用户询问真实类别，未臆造新训练标签。
- 同缓存预测事后诊断仅删除触边≤1px的phone候选，三模型第二次手机missing恢复52秒，长杯片额外手机事件和空场片手机事件消失，杯事件不变。只是开发诊断，未改正式运行或默认模型；[结果](../docs/scene-development-results-20260920.md)与[脱敏统计](evaluations/scene-development-20260920.json)。
- 修复scripts/train_scene_model.py选优标签与冻结dataset业务标签category/bbox/iscrowd多重集一致性，保留非业务训练标签。实际 `.export-venv/bin/python -m pytest -q tests/test_train_scene_model.py`：25 passed（root复跑0.36秒），R04现有42条清单验证通过，Ruff/diff检查通过。未重复全库测试或称模型质量验收通过。私人视频/图片/逐帧预测不提交，保留原工作树改动。


## 2026-09-20 19:04 +08 — 忽略指定银色物体

- 用户明确“银色物体忽略不管即可”。已写入AGENTS/GOALS，更新PLANS和本轮结果/context：取消该物体的类别确认、纠错训练及补录，不以该对象本身误识别阻塞验收。仅针对已指出的底部银白物体，不扩大为所有银色对象或触边手机。
- 原始预测和事件干扰保留；本次仅规则/文档更新，未运行新推理、未改运行过滤器/生产模型，不将忽略写成修复成功。diff检查通过，保留已有无关工作树修改。


## 2026-09-20 19:20 +08 — 指定银白物体运行忽略与事件回归

- 用户要求开始后续工作。实现固定小区域＋底边的phone候选忽略，保留其他边缘phone及cup/bottle；配置默认关闭，页面可开启，仅1920×1080完整画面允许启动，实际异尺寸不套用过滤。启停参数纳入视角基线，切换不补虚假missing。未改本机1280×720/.env或正式权重。
- 真实缓存401采样×3模型经过实际wrapper与MemoryStore（无伪造证据图）；杯4/4完整移出保持，正常手机两次移出23/52秒记录，第二次恢复，空场/杯片额外phone事件清除。另12次真实模型+过滤器新推理与缓存一致。1px容差残留两框后修订为2px，两版输出保留；属于开发修正非独立验收。
- 检查顺序实际帧纠正旧phone稳定区间的17/18秒：前者手遮挡、后者已移出；旧31正样本数字不继续当准确率分母，未改历史记录。R03正常12秒单次漏检仍保留，不为豁免情形训练。
- 最终 `.venv/bin/pytest -q`：422 passed、1 skipped、5 deselected/14.82秒；Ruff和diff通过。中途集成发现设置元组顺序/旧测试索引不一致，已统一并补UI启用状态/无变更不重启测试后重新全量通过。未真实USB/API/Windows，未独立验收。使用/证据见[实现报告](../docs/silver-object-ignore-20260920.md)。当前无需R06，保留现有正式配置及R03/R04/R05对照候选，等待独立正常操作验证。


## 2026-09-20 19:50 +08 — 用户现场漏检反馈：测试配置核查

- 用户反馈低置信度及杯/手机漏检。只读检查页面实际启用设置：请求1280×720、完整画面、杯复查开、银色忽略关，实际输入1920×1080；页面当前可见杯但无检测。配置model_path为models/yolo26n-e2e.onnx，文件SHA为9c60d351bb2865a8169d0590c07c905b372e81e4a846a8ff920d244955e2516c，对应原始COCO官方导出，不是R03/R04/R05微调候选。此前开放最终测试未核对候选身份，准备不足，本轮不作为微调候选的最终验收结果。
- 只读SQLite截面11:45:02—11:49:14 UTC共243观察（242fresh、1stale），cup共4框置信度0.410—0.462，phone7框0.355—0.568；无逐帧真值，不作漏检率。界面历史手机52%来自09-17，并非当前帧。
- 对已知development手机片顺序第720帧（名义12秒）用真实ONNX同帧探针，置信度降到0.01：原始模型无phone候选，R04正确完整手机框0.96185，另有底角phone0.49254。单帧仅佐证模型配置差异，不代表实时现场或独立精度；不建议靠盲降门槛修复。
- 未停止用户观察、未切模型/调阈值。下一步应使用隔离数据目录和明确哈希的候选测试配置，并在页面显示模型身份，再重新开始候选验收；当前实际1080不能归因于请求720分辨率。


## 2026-09-20 20:01 +08 — 物品模型选择与实际身份显示

- 用户要求侧边栏新增类似饮水模型的识别模型选择。新增原始/R03/R04/R05预设与沿用配置选项，区分待启用和实际加载身份，实验标记可见；显式预设启动前验证固定SHA（包含缓存复用前），更换模型停止旧观察、重建检测器并重置事件基线。缺失/失败不发布新身份，不写.env。见[使用说明](../docs/object-model-selector.md)。
- `.venv/bin/pytest -q` 最终428 passed、1 skipped、5 deselected/20.45秒；Ruff通过。AppTest覆盖仅选择不生效、点击启动停止重建、缺失文件不伪报成功；修正原fake camera测试以准备测试模型文件，测试不依赖私人权重。
- 真实YoloOnnxDetector依次校验并加载original/R03/R04/R05成功。重启本地8501页面服务并刷新浏览器，实际确认五项下拉、默认原始模型、当前实际启用为无；当前观察已停止，未自动切换候选或开启摄像头。未真实视觉质量/API/Windows验收。


## 2026-09-20 20:14 +08 — 在座反馈与空座/未知跳变只读核查

- 用户报告一直在座，页面却频繁空座/未知。浏览器当前预览为桌面、手机、杯子与空黑椅，未见人；已询问用户是否坐在该椅，尚未收到答复。此矛盾可能涉及取景/设备/输入，不能据预览反驳用户或直接归因为模型漏人；未改摄像头或重启观察。
- 只读SQLite最近250行为区间（12:12:19.879—12:13:51.850 UTC）213 unknown、37 empty；160个区间超过0.25秒。另约10秒轮询取得48条不同时间观察：30 running/unknown、16 running/empty、2 stale/unknown且error为推理完成超过新鲜限。轮询可能漏中间观察，不作完整发生率。
- BehaviorTimelineV2即使fresh=true也在相邻时间戳间隔>0.25秒时重置；恢复要连续0.3秒确认姿态。用随后最近250个真实区间结束时间重放恒定raw empty、fresh=true：235 unknown/15 empty，证明时间间隔自身足以产生跳变（模拟验证机制，非现场原始分类输出）。不能把running+fresh unknown都当分类低置信。
- 当前原始姿态分数未持久化，未确认empty来自哪些现场原始帧。代码共享订阅按递增sequence取帧，无同订阅重复消费；预处理与清单一致为320全帧letterbox。保持门限和模型，先确认用户所在座位是否入镜，再决定现场诊断与负载修复，不以延长过期门限掩盖问题。未进行质量验收或宣布修复。


## 2026-09-20 21:22 +08 — 已入镜仍未知：现场原始分类与延迟定位

- 用户确认现已坐到摄像头前；浏览器21:05及之后预览明确有人在座，排除本次未入镜。临时本地诊断读取detector.snapshot及最新保存/规则处理耗时，保留一张本地画面；原始私人数据仅data/behavior-live-diagnosis-20260920，不提交。采样探针已移除，服务已干净重启。
- 诊断某截面去重594个sequence全部raw seated，典型confidence>0.9998。该截面推理耗时p50/p95/max约212/448/2136ms；最近保存回调耗时80/414/2603ms；最近规则处理3/117/364ms。回调耗时与模型snapshot异步读取，非逐帧配对、非全流分位数。足以证明原始模型识别在座但处理链路超过250ms保护限，不能宣称模型错认空座或要求重训。
...
## 2026-09-20 23:54 +08 — 保存当前版本

- 用户要求先提交当前版本。性能修复代码已在1a06a65提交并同步；本次补充保存现有项目介绍、生成式概念配图及提示词、本地模型文件清单和此前未提交的09-16配置记录，正文保持原样。清单只包含路径/大小，不包含模型二进制；配图已目视确认是功能概念图。
- 本次没有业务代码变化，沿用1a06a65的443项回归结果，不声称重新执行。提交前检查差异与凭据模式；私人运行数据、模型权重和.env仍排除。

## 2026-09-21 00:17 +08 — 当前网页卡顿只读分析

- 用户询问卡顿能否优化。核对当前源码、安装框架与真实SQLite：物品最近60条全running、采样间隔中位1.007秒、推理中位61.77ms、实际720p；行为约10秒轮询100条不同观察全running，入库年龄中位49.45ms/最大72.53ms。轮询不是完整流验收。
- 预览绑定物品推理结果且页面1Hz刷新，构成明确的低帧率观感来源；统计每5秒全量解析当天历史是另一减负点。只读19085区间查询10次中位21.77ms/最大99.37ms，不含正式完整统计与浏览器渲染。
- [分析与实施顺序](../docs/web-latency-analysis-20260921.md)：优先解耦预览，其次统计聚合及闲置对话刷新。浏览器接口长时阻塞，未取得交互性能证据，不把工具阻塞算应用卡顿。此轮仅分析，未改源码/配置或重启观察，无API/训练；未运行代码测试。

## 2026-09-21 00:23—00:42 +08 — 流畅预览保留物品框与统计减负

- 用户要求实施优化且保留框选。新增约5fps、最多960宽JPEG预览，独立于原物品推理/证据；显示最近真实框，间隔内光流跟踪并明确tracked/检测年龄，失踪、过期、断连清除。预览不持久化或生成业务事件，未增加真实模型调用。两位Sol分别完成显示跟踪与统计聚合，root集成、审查及实机验证。
- 统计改为SQLite微秒精度聚合，保留跨日/DST/unknown等语义，修正unixepoch兼容及小数跨秒舍入风险；闲置对话停止重复重绘，高频预览不查询数据库。
- 全套450 passed、1 skipped、5 deselected/17.60秒，Ruff与9文件格式检查通过；末次窄屏排版调整后UI 12 passed/7.05秒。首轮448通过/1失败为fake camera未实现新状态接口，修正后保留断连断言并通过。
- 真实8501/R05/720p＋原行为模型：截图确认杯和手机绿色tracked框；约10秒49次图片URL更新（约5fps），175采样中153完成加载，不能等同每次均绘制完成。30秒轮询298个不同行为记录全running，入库帧龄中位49.42ms/最大92.26ms；非完整流长期验收。浏览器接口两次长时响应不作为应用卡顿证据。
- 正常TERM旧服务并核对退出，最终进程恢复R05选择，不改.env/权重。没有主动API调用/训练/Windows验收。实现、采样边界和回退见[分析与结果](../docs/web-latency-analysis-20260921.md)。

## 2026-09-21 01:06 +08 — 恢复易读框选样式

- 用户要求恢复上一版框选样式。改为与原检测预览一致的Supervision分类颜色、实心标签底色/高对比文字及类别、置信度、区域格式。跟踪标记与年龄移至已有预览下方说明，保留流畅预览和跟踪逻辑。
- `.venv/bin/pytest -q tests/vision/test_preview.py tests/vision/test_worker.py`：17 passed/2.21秒；Ruff/格式/diff通过。只调整既有坐标断言以兼容分类配色，无新算法测试或全量复跑。正常重启8501并恢复R05/720p设置，无.env或权重改动。

## 2026-09-21 01:57 +08 — 浅色工作台界面改版与验证

- 用户要求按照已生成效果图调整实际界面并测试。实现五入口导航、预览/对话约60:40双栏、当前物品表格、真实最近动态、参数集中设置、统一字体/留白/青绿主题和窄屏单列。保留全部原功能入口、实验/过期/未连接标识及按需证据；不将效果图示例写入真实记录。说明见[工作台界面](../docs/ui-workbench-20260921.md)，决策见[上下文](context/ui-workbench-20260921.md)。
- 两位Sol分别负责主题与测试/只读审查，root集成、修复并实机复测。会话偏好独立于运行设置，跨页不丢失，点击开始才应用；保留单Runtime/后台线程和原刷新频率。修正完整行为/笔记本设置元组比较，避免相同参数误重启。对话Future跨页只提交一次，完成后恢复输入。
- `.venv/bin/pytest -q`最终457 passed、1 skipped、5 deselected/20.63秒；此前全量455及457通过分别为追加异步测试前与导航片段修复前，均不代替最终结果。最终定向UI19项通过/9.28秒；子Agent整个tests/ui为39项通过/15.25秒。`.venv/bin/ruff check .`、四个修改Python文件format check、git diff检查通过。
- 实际Chrome页面验证1440×1000、900×1000、390×844布局。修复真实DOM选择器、桌面规则覆盖窄屏宽度、占位图溢出、展开按钮被隐藏及透明工具栏遮挡；最终900宽两面板均672px满宽，390宽页面scrollWidth=390、占位图330.4px且导航按钮可重新展开。宽屏面板等高；截图通过本地浏览器工具回显，未提交私人截图。
- AppTest全页运行未暴露的片段快捷导航问题在真实浏览器发现：加入请求标记与全应用重绘后，「查找物品」实际进入记忆且侧栏同步选中，「设置提醒」进入提醒；实际检查关注/情境规则、行为/笔记本、设备/调用记录入口。审查中「查看全部」语义和队列/具体日上限丢失已修正，见[审查](code_review/ui-workbench-20260921.md)。
- 本机正常重启服务并恢复R05、720p、杯复查开启/银色忽略关闭及原行为模型，真实预览960×540 JPEG加载、杯/手机框与当前列表可见；停止后显示未知且清空当前物品，恢复后继续running。30.08秒只读轮询取得289条不同行为观察全running，读取时帧龄最大173.34ms；轮询非完整流，也不是长期性能或识别准确率验收。启动暖机过期保留，不放宽保护门限。
- 首次热加载遇旧Runtime资源锁，用正常停止/重启解决，未删除数据库或锁文件。最终服务仅通过进程环境沿用此前实际R05，`.env`、权重和识别阈值未修改；浏览器临时尺寸已复原，工作台保留打开并恢复观察。未主动调用云API、训练或Windows实机验收；今日调用页面显示请求/自动运行均0。提交前远程main与本地HEAD同为86a6a66。
