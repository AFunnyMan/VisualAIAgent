...
- 最终离线92项、真实API独立3项及macOS/Windows托管CI均通过。
- 最终严格视频短测：出现/合成空白未检测到均由真实Agent提醒，延迟约2.78/3.25秒；故障、重复事件与重启检查通过，回答事实锚点和请求数对账通过。
- 已连接待机回放实际1800.013秒，360次检查全部有效；新增Agent运行/HTTP请求均为0，峰值RSS356.91MiB、平均CPU36.56%（单核100%）。长测进程启动于最后一轮答案提示增强之前，仅用于验证未改变的持续视觉/等待链路；最终答案质量以上一项严格短测为准。
- 原始API请求、失败与重跑均计入，总计115次，input163408/output5799 Tokens，原价估算约0.03321元。明细与完整证据边界见 [进度日志](../harness/build-log.md)。
