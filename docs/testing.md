...
GitHub Actions 的 [offline.yml](../.github/workflows/offline.yml) 使用 macOS/Windows 托管 runner 安装锁定环境并运行上述离线检查。工作流文件存在不代表其已经运行；托管 Windows 也不等同 Windows 11 USB 实机。
...
步骤统一放在 [用户辅助验收](user-acceptance.md)，包括：显式启用 `VAA_RUN_LIVE_API=1` 的真实 SDK 测试、三类各十次放入/移出、两秒遮挡、候选、断连/停止/重启、正常/降级通知和两平台各三十分钟运行。
